...
<div class="wall-item-outside-wrapper {{$item.indent}}{{$item.previewing}}{{if $item.owner_url}} wallwall{{/if}}" id="wall-item-outside-wrapper-{{$item.id}}" >
	...
	<div class="wall-item-content-wrapper {{$item.indent}}" id="wall-item-content-wrapper-{{$item.id}}" >
		<div class="wall-item-info{{if $item.owner_url}} wallwall{{/if}}" id="wall-item-info-{{$item.id}}">
			...
			{{/if}}
			<div class="dropdown wall-item-photo-wrapper{{if $item.owner_url}} wwfrom{{/if}}" id="wall-item-photo-wrapper-{{$item.id}}" 
				...
				<a href="#" class="dropdown-toggle wall-item-photo-menu-button" data-toggle="dropdown" rel="wall-item-photo-menu-{{$item.id}}" id="wall-item-photo-menu-button-{{$item.id}}"><span class="icon-chevron-down"></span></a>
		                <div class="dropdown-menu wall-item-photo-menu" id="wall-item-photo-menu-{{$item.id}}">
			                    <ul>
		...
		<div class="wall-item-author">
				...
				<div class="wall-item-ago"  id="wall-item-ago-{{$item.id}}" title="{{$item.isotime}}"><span class="icon-time"></span> {{$item.localtime}}</div>				
		</div>			
		...
		<div class="wall-item-tools" id="wall-item-tools-{{$item.id}}">
			{{if $item.like}}
				<a href="#" class="icon-thumbs-up item-tool" title="{{$item.like.0}}" onclick="dolike({{$item.id}},'like'); return false"></a>
			{{/if}}
			{{if $item.dislike}}
				<a href="#" class="icon-thumbs-down item-tool" title="{{$item.dislike.0}}" onclick="dolike({{$item.id}},'dislike'); return false"></a>
			{{/if}}
			{{if $item.share}}
				<a href="#" class="icon-retweet item-tool" title="{{$item.share.0}}" onclick="jotShare({{$item.id}}); return false"></a>
			{{/if}}
			{{if $item.plink}}
			...
			<div class="wall-item-delete-wrapper" id="wall-item-delete-wrapper-{{$item.id}}" >
				{{if $item.drop.dropping}}<a href="item/drop/{{$item.id}}" onclick="$('wall-item-outside-wrapper-{{$item.id}}').css('background','red');return confirmDelete();" class="icon-trash icon-large" title="{{$item.drop.delete}}" onmouseover="imgbright(this);" onmouseout="imgdull(this);" ></a>{{/if}}
			</div>
